...
class InheritPOSOrder(models.Model):
	...
	woo_status = fields.Selection([("pending", "Pending"), ("processing", "Processing"),
                                   ("on-hold", "Ship-to Customer"), ("completed", "Completed"),
                                   ("cancelled", "Cancelled"), ("refunded", "Refunded")],
                                   copy=False, tracking=7)

	def button_posting(self, orderid):
		self.env['sale.order'].search([('id', '=', orderid)]).write({'state':'sale'})
# 		self.env['sale.order'].search([('id', '=', orderid)]).write({'woo_status':'completed'})
		return True 
	...
	def search_all_sale_order(self,config_id,l_date):
		...
		if config.load_orders_days > 0:
			sale_orders = self.env['sale.order'].search([('date_order','>=',last_day),('woo_status', '=', 'processing')])
		else:
			sale_orders = self.env['sale.order'].search([('woo_status', '=', 'processing')])
		for s in sale_orders:
			vals1 = {
				...
				'woo_status': s.woo_status,
				'woo_order_number': s.woo_order_number,
				'x_phone': s.x_phone,
				'x_contact_address_complete': s.x_contact_address_complete,
			}
