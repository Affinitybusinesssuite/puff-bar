odoo.define('bi_pos_import_sale.import_sale', function(require) {
	...
	models.load_models({
		...
		fields: ['name','partner_id','user_id','amount_untaxed','state',
				 'order_line','amount_tax','amount_total','company_id','date_order','woo_status','woo_order_number', 'x_phone', 'x_contact_address_complete'],
		...
		show: function(options) {
			...
			this.$('.client-list-contents').delegate('.sale-order','click',function(event){
				...
				$.each(list_of_qty, function(index,value) {
					...
					entered_item_qty.val(orderlines[index]['product_uom_qty']);
				});				
			});
...
// 			var selectedOrder1 = self.pos.get_order();
// 				console.log("Order", selectedOrder1);
				
		rpc.query({
		      model: 'pos.order',
		      method: 'button_posting',
		      args: [1, id], //here pass parameters of python function
			}).then(function(output) {
