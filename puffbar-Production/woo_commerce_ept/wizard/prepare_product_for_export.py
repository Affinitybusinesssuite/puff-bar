...
class PrepareProductForExport(models.TransientModel):
    ...
    def export_direct_in_woo(self, product_templates):
        ...
        for variant in variants:
            ...
            if product_template.categ_id:
                woo_category_dict = self.create_categ_in_woo(product_template.categ_id, woo_instance.id, woo_category_dict)  # create category
    ...
    def import_products_from_csv(self):
        ...
        for record in file_data:
            ...
            if not woo_template:
                if categ_obj:
                    woo_category_dict = self.create_categ_in_woo(categ_obj, instance_id.id, woo_category_dict)  # create category
    ...
    def update_category_info(self, categ_obj, instance_id):
        ...
        """
        woo_product_categ = self.env['woo.product.categ.ept']
        woo_categ_id = woo_product_categ.search([('name','=',categ_obj.name),('woo_instance_id','=', instance_id)], limit=1)
        if woo_categ_id and woo_categ_id.name != categ_obj.name:
